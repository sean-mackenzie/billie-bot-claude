...
| `serial_port` | `/dev/serial/by-id/usb-Silicon_Labs_CP2102_..._0001-if00-port0` | RPLidar A1 USB-UART device on the Jetson. Stable by-id path, matching the `base_bridge` convention — plug-in order and `ttyUSBn` numbering are irrelevant (Appendix B-9, GAP-20). Set in `billiebot_bringup/config/lidar.yaml`, not inline in the launch file |
...
| `model_path` | `''` | Path to the YOLOv8n blob. **Empty default ⇒ in real mode the node now logs an ERROR and exits non-zero (GAP-11, resolved 2026-07-31) rather than idling silently.** `perception.yaml` sets a real deployment path and `07_oakd.launch.py` now loads it (Appendix B-10/B-11) |
...
| B-10 | **Resolved 2026-07-31 (GAP-11 PR).** ~~`perception.yaml` never loaded by the ladder~~ — rungs 07/09/10 now load `config_file` ahead of the `mock` override, same pattern rung 11 already used for audio. No GAP number was ever assigned to this row | `07_oakd.launch.py`, `09_thermal.launch.py`, `10_noir.launch.py` | (historical) silent config-drift trap — editing `perception.yaml` had no effect on the ladder | Done — see `DISCREPANCY_RESOLUTION_PLAN.md` §GAP-11 |
| B-11 | **Split.** The `oakd_dog_detector` half is **resolved 2026-07-31 (GAP-11).** ~~`model_path` defaults `''` → real mode = error log + zero output~~ — real mode now validates `model_path` before touching `depthai` and `sys.exit(1)`s (with a clear ERROR) on empty/missing path, `depthai` import failure, or any pipeline/device exception; `perception.yaml`'s path is now actually wired in via B-10's fix. The `audio_classifier` half (`model_path` defaults `''` for the YAMNet path) is a separate node/asset and **remains open** — not part of GAP-11's recommended fix or its GitHub issue. The bbox-truncation defect (DepthAI `det.xmin/ymin/xmax/ymax` are normalized floats; `int()` of them zeroes the bbox fields, position/depth unaffected) is **also still open** — GAP-11 never covered it | `oakd_dog_detector.py` `init_depthai_pipeline` (done) `real_detect` (bbox bug, open); `audio_classifier.py` (open) | (historical, OAK-D half) first hardware run of rung 07 would have silently produced nothing — now fails loud instead; bbox fields still useless for downstream motion-energy features (design §5.3 ACTIVE heuristic); audio's `model_path` still needs the same treatment | OAK-D half: done — see `DISCREPANCY_RESOLUTION_PLAN.md` §GAP-11. Remaining: apply the same fail-loud pattern to `audio_classifier`; scale bbox by preview size |
