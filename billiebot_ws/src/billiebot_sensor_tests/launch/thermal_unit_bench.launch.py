"""UT-THM-01 (stream test) and UT-THM-02 (target/background contrast -- same launch file,
default duration_sec:=90 covers the 15+30+15 s ambient/target/ambient sequence in one
recording) share this launch file.

Starts the production thermal_node UNMODIFIED (mock:=false, plan section 5b -- no
production change), the test-only colorizer, rate monitor, ground-truth marker, rosbag2
recording, and foxglove_bridge.
"""
...
def generate_launch_description():
    ...
    actions.append(DeclareLaunchArgument('mock', default_value='false'))
    actions.append(manifest_bootstrap_action('UT-THM-01', 'thermal', 'MLX90640'))

    actions.append(replicate_production_node(
        'billiebot_perception', 'thermal_node', production_config,
